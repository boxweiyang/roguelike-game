...
class SkillManager {
    ...
    getSkillCooldown(skillData) {
        ...
        if (this.passiveItems['cooldown_reduction']) {
            cooldown *= (1 - this.passiveItems['cooldown_reduction'].data.effect.cooldownReduction);
        }
        
        if (this.passiveItems['attack_speed']) {
            cooldown *= (1 - this.passiveItems['attack_speed'].data.effect.cooldownReduction);
        }
    ...
    calculateDamage(baseDamage, skillType) {
        ...
        if (skillType.includes('lightning') && this.passiveItems['lightning_damage']) {
            damage *= (1 + this.passiveItems['lightning_damage'].data.effect.lightningDamage);
        }
    ...
    executeAutoAOE(player, enemies, levelData, damage) {
        ...
        // 选择最近的敌人
        const targets = this.findNearestEnemies(player, enemies, lightningCount, range);
        ...
        this.projectiles = this.projectiles.filter(proj => {
            if (proj.homing && proj.target) {
                ...
                if (dist < proj.speed) {
                    ...
                    this.createExplosionEffect(proj.target.x, proj.target.y, proj.explosionRadius);
                    return false;
                }
    ...
    updateOrbitals(player, enemies) {
        this.orbitals.forEach(orb => {
            ...
            enemies.forEach(enemy => {
                ...
                const dist = Math.sqrt(
                    Math.pow(enemy.x - orbX, 2) + 
                    Math.pow(enemy.y - orbY, 2)
                );
