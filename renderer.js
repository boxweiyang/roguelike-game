...
function initRender() {
  ...
  neonRenderer = new NeonRenderer(gameCtx); // 初始化霓虹渲染器
  console.log("渲染系统初始化完成（含霓虹效果）");
}
...
function renderGrid(ctx) {
  ...
  const startX =
    Math.floor(
      gameState.player.x - CONFIG.CANVAS_WIDTH / CONFIG.TILE_SIZE / 2,
    ) - 1;
  const startY =
    Math.floor(
      gameState.player.y - CONFIG.CANVAS_HEIGHT / CONFIG.TILE_SIZE / 2,
    ) - 1;
  ...
  ctx.strokeRect(
    0,
    0,
    CONFIG.ARENA_SIZE * CONFIG.TILE_SIZE,
    CONFIG.ARENA_SIZE * CONFIG.TILE_SIZE,
  );
}
...
function renderEnemies(ctx, enemies) {
  for (const enemy of enemies) {
    ...
    const size = CONFIG.TILE_SIZE * enemy.size * 0.45;
